<?php
/**
 * Documents Schema Attributes Mapping Workspace View Card.
 *
 * Provides the user interface configuration layout screen for managing custom attributes
 * assigned exclusively to Document objects vault workflows.
 *
...
$docs_specific_attributes = array_filter( $dynamic_fields, function( $field ) {
    return isset( $field['object_type'] ) && $field['object_type'] === 'vaplm_document';
});
...
<div class="vaplm-card" style="margin-top: 0; padding: 25px;">
    <div style="margin-bottom: 20px; border-bottom: 1px solid #f0f0f1; padding-bottom: 10px;">
        <h3 style="margin: 0; font-size: 14px; text-transform: uppercase; color:#2271b1;">📄 <?php esc_html_e( 'Documents Vault Custom Attributes Schema Registry', 'va-plm-admin-suite' ); ?></h3>
        <p class="description" style="margin-top: 5px;">
            <?php esc_html_e( 'Provision customized documentation tracking parameters (e.g., Drawing Scale, Confidentially Level, Department Owner) assigned to the static General Properties workspace form view of the Documents Vault specification sheet.', 'va-plm-admin-suite' ); ?>
        </p>
    </div>
    ...
    <form method="post" action="">
        ...
        <input type="hidden" name="vaplm_field_scope_tab" value="vaplm_document" />

        <table class="wp-list-table widefat fixed striped vaplm-data-table" style="width: 100%; border-collapse: collapse;" id="vaplm-docs-schema-attributes-table">
            <thead>
                <tr>
            ...
            <tbody>
                <?php if ( ! empty( $docs_specific_attributes ) ) : ?>
                    <?php foreach ( $docs_specific_attributes as $key => $config ) : ?>
                        <tr data-field-key="<?php echo esc_attr( $key ); ?>">
                            <td style="padding: 12px; vertical-align: middle;">
                                ...
                                <input type="hidden" name="vaplm_dynamic_fields[<?php echo esc_attr( $key ); ?>][object_type]" value="vaplm_document" />
                ...
                <?php else : ?>
                    <tr class="vaplm-empty-schema-fallback-row">
                        <td colspan="6" style="text-align: center; color: #646970; padding: 20px; font-style: italic;">
                            <?php esc_html_e( 'No customized attributes registered against the Documents post vault schema topology yet.', 'va-plm-admin-suite' ); ?>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
        ...
        <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #f0f0f1; display: flex; justify-content: space-between; align-items: center;">
            <button type="button" class="button button-secondary vaplm-add-schema-row-btn" data-target="vaplm-docs-schema-attributes-table" data-object-type="vaplm_document">
                ➕ <?php esc_html_e( 'Append New Custom Document Field', 'va-plm-admin-suite' ); ?>
            </button>
            <?php submit_button( __( 'Save Documents Schema Attributes Changes', 'va-plm-admin-suite' ), 'primary large', 'vaplm_save_docs_schema_btn', false ); ?>
        </div>
    </form>
</div>