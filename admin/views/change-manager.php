<?php
/**
 * Change Management Schema Attributes Mapping Workspace View Card.
 *
 * Provides the user interface configuration layout screen for managing custom attributes
 * assigned exclusively to Change Order tracking workflows.
 *
...
$change_specific_attributes = array_filter( $dynamic_fields, function( $field ) {
    return isset( $field['object_type'] ) && $field['object_type'] === 'vaplm_change_order';
});
...
<div class="vaplm-card" style="margin-top: 0; padding: 25px;">
    <div style="margin-bottom: 20px; border-bottom: 1px solid #f0f0f1; padding-bottom: 10px;">
        <h3 style="margin: 0; font-size: 14px; text-transform: uppercase; color:#2271b1;">🔄 <?php esc_html_e( 'Change Management Custom Attributes Schema Registry', 'va-plm-admin-suite' ); ?></h3>
        <p class="description" style="margin-top: 5px;">
            <?php esc_html_e( 'Provision customized engineering change attributes (e.g., Change Reason, Priority Code, Production Disposition) assigned to the static General Properties workspace form view of Engineering Change Orders (ECO).', 'va-plm-admin-suite' ); ?>
        </p>
    </div>
    ...
    <form method="post" action="">
        ...
        <input type="hidden" name="vaplm_field_scope_tab" value="vaplm_change_order" />

        <table class="wp-list-table widefat fixed striped vaplm-data-table" style="width: 100%; border-collapse: collapse;" id="vaplm-change-schema-attributes-table">
            <thead>
                <tr>
                    ...
                    <th style="padding: 10px; font-weight: 600; width: 20%;"><?php esc_html_e( 'LOV Dictionary Target Mapping', 'va-plm-admin-suite' ); ?></th>
            ...
            <tbody>
                <?php if ( ! empty( $change_specific_attributes ) ) : ?>
                    <?php foreach ( $change_specific_attributes as $key => $config ) : ?>
                        <tr data-field-key="<?php echo esc_attr( $key ); ?>">
                            <td style="padding: 12px; vertical-align: middle;">
                                ...
                                <input type="hidden" name="vaplm_dynamic_fields[<?php echo esc_attr( $key ); ?>][object_type]" value="vaplm_change_order" />
                            ...
                            <td style="padding: 12px; vertical-align: middle;">
                                <select name="vaplm_dynamic_fields[<?php echo esc_attr( $key ); ?>][field_type]" class="vaplm-schema-attribute-ui-selector-trigger" style="width: 100%;">
                                    ...
                                    <option value="textarea" <?php selected( $config['field_type'] ?? '', 'textarea' ); ?>><?php esc_html_e( 'Multi-Line Paragraph Block', 'va-plm-admin-suite' ); ?></option>
                ...
                <?php else : ?>
                    <tr class="vaplm-empty-schema-fallback-row">
                        <td colspan="6" style="text-align: center; color: #646970; padding: 20px; font-style: italic;">
                            <?php esc_html_e( 'No customized attributes registered against the Change Management post schema topology yet.', 'va-plm-admin-suite' ); ?>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
        ...
        <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #f0f0f1; display: flex; justify-content: space-between; align-items: center;">
            <button type="button" class="button button-secondary vaplm-add-schema-row-btn" data-target="vaplm-change-schema-attributes-table" data-object-type="vaplm_change_order">
                ➕ <?php esc_html_e( 'Append New Custom Change Field', 'va-plm-admin-suite' ); ?>
            </button>
            <?php submit_button( __( 'Save Change Schema Attributes Changes', 'va-plm-admin-suite' ), 'primary large', 'vaplm_save_change_schema_btn', false ); ?>
        </div>
    </form>
</div>